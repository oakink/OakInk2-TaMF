...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, loss_callback=None):
        """
        ...
        batch_size, nfeats, _, nframes = x_start.shape
        batch = model_kwargs["batch"]
        mask = batch["mask"]
        mask = mask.reshape((batch_size, 1, 1, nframes))
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if loss_callback is not None:
                ...
                extra_loss, extra_loss_dict = None, None
        else:
            raise NotImplementedError(self.loss_type)

        return terms, (extra_loss, extra_loss_dict)
