...
from ..dataset.pose_repr_sample import GeneratedPoseReprSampleAdaptor, GuassianPerturbSampleAdaptor, IdentitySampleAdaptor
from ..dataset.action_adapter import ActionRecognitionAdapter
from ..dataset.collate import SegmentCollate
from ..model.segment_encoder import SegmentEncoder
from ..model.segment_encoder_loss import SegmentEncoderLoss
from ..model.segment_encoder_param import reg_model_param, reg_loss_param
...
from ..util.step_id_util import handle_step_epoch

from .param import reg_mano_param
...
def reg_entry(config_reg: ConfigRegistry):
    ...
    config_reg.register(
        ...
        source=ConfigEntrySource.COMMANDLINE_OVER_CONFIG,
        cmdpattern=ConfigEntryCommandlineSeqPattern.COLON_SEP,
        default=["?(file:common/generate_split/main/split/val.txt)"],
    )
    config_reg.register(
        "cache_dict_filepath",
        prefix=PARAM_PREFIX__VAL,
...
def run(rank, world_size, ckpt_cfg, run_cfg):
    ...
    train_identity_sample_dataset = IdentitySampleAdaptor(train_dataset_)
    train_pose_repr_sample_dataset = GeneratedPoseReprSampleAdaptor(
        train_dataset_, run_cfg["train"]["data"]["pose_repr_sample_dir_list"])
    train_gaussian_perturb_dataset = GuassianPerturbSampleAdaptor(train_dataset_,
                                                                  run_cfg["train"]["data"]["gaussian_perturb_range"])
    train_dataset_ = torch.utils.data.ConcatDataset(
        [train_identity_sample_dataset, train_pose_repr_sample_dataset, train_gaussian_perturb_dataset])
    train_dataset = ActionRecognitionAdapter(train_dataset_)
    ...
    if not rank:
        ...
        val_dataset = ActionRecognitionAdapter(val_dataset_)
        ...
        test_dataset = ActionRecognitionAdapter(test_dataset_)
    ...
    train_dataloader = torch.utils.data.DataLoader(
        ...
        drop_last=True,
        pin_memory=True,
        persistent_workers=run_cfg["runtime"]["num_worker"] > 0,
        worker_init_fn=None,
    )
    if not rank:
    ...
    model_ = SegmentEncoder(
        train_dataset.max_action,
    ...
    model_loss = SegmentEncoderLoss().to(device)
    ...
    for epoch_id in range(num_epoch):
        ...
        for batch_id, batch in enumerate(train_dataloader):
            ...
            # replace pose_repr with sample pose repr
            batch["_pose_repr"] = batch["pose_repr"]
            batch["pose_repr"] = batch["sample_pose_repr"]
        ...
        if not rank:
            ...
            if (commit and (record_freq is not None and record_freq != -1) and
                (epoch_id == 0 or epoch_id % record_freq == record_freq - 1 or epoch_id == num_epoch - 1)):
                model_weight_path = ckpt.handle_save_path(f"?(ckpt_path)/save/model_{epoch_id:0>4}.pt",
                                                          ckpt_path=ckpt_path)
                save_state(model.state_dict(), model_weight_path, remove_prefix="module", filter_out="clip_model")
                optimizer_weight_path = ckpt.handle_save_path(f"?(ckpt_path)/save/optimizer_{epoch_id:0>4}.pt",
                                                              ckpt_path=ckpt_path)
        ...
        if not rank:
            ...
            if (val_freq is not None and val_freq != -1) and (epoch_id == 0 or epoch_id % val_freq == val_freq - 1 or
                                                              epoch_id == num_epoch - 1):
                with torch.no_grad():
                    val_pbar = tqdm.tqdm(
                    ...
                    for val_batch_id, val_batch in enumerate(val_dataloader):
                        step_id = handle_step_epoch(epoch_id, val_freq) * len(val_dataloader) + val_batch_id
                    ...
                    _logger.info("val epoch %04d conclude | loss: %f", epoch_id, val_loss.item())
            ...
            if (test_freq is not None and test_freq != -1) and (epoch_id == 0 or epoch_id % test_freq == test_freq - 1
                                                                or epoch_id == num_epoch - 1):
                with torch.no_grad():
                    test_pbar = tqdm.tqdm(
                    ...
                    for test_batch_id, test_batch in enumerate(test_dataloader):
                        step_id = handle_step_epoch(epoch_id, test_freq) * len(test_dataloader) + test_batch_id
                    ...
                    _logger.info("test epoch %04d conclude | loss: %f", epoch_id, test_loss.item())
...
def main():
    ...
    run_cfg["runtime"]["device_id__ori"] = run_cfg["runtime"]["device_id"]
