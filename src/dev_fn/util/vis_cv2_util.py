...
def debug_img(img, reverse_channels=False):
    if reverse_channels:
        img = img[:, :, [2, 1, 0]]
...
def color_depth(depth_img, colormap="plasma", depth_min=None, depth_max=None):
    import numpy as np
    import matplotlib

    depth_avail = depth_img > 0.0
    depth_image_normalized = np.zeros_like(depth_img)
    if depth_min is None:
        depth_min = np.min(depth_img[depth_avail])
    if depth_max is None:
        depth_max = np.max(depth_img[depth_avail])
    depth_image_normalized[depth_avail] = (np.clip(depth_img[depth_avail], depth_min, depth_max) - depth_min) / (
        depth_max - depth_min
    )
    ...
    colored_depth_image = colormap(depth_image_normalized)
    colored_depth_image[~depth_avail] = 0.0
    colored_depth_image = (colored_depth_image[:, :, :3] * 255).astype(np.uint8)

    return colored_depth_image


def debug_depth(depth_img, colormap="plasma", depth_min=None, depth_max=None):
    colored_depth_image = color_depth(depth_img, colormap=colormap, depth_min=depth_min, depth_max=depth_max)
    debug_img(colored_depth_image, reverse_channels=True)
